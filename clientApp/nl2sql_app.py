...
from graph_generator import get_plotly_graph, get_empty_graph
import oracledb
from llm_handler import create_user_message, create_assistant_message, chat_conversion,get_llm_sql, check_sql_equiv, get_prompt_equiv, seek_clarification, check_graphing_request, read_file_to_string, get_sql_prompt, seek_explanation, seek_intent
import pickle
import difflib
...
from helper_methods import initialize_query_variables, set_chat_cache, get_chat_cache, set_conversation_cache, get_conversation_cache,set_session_data
from helper_methods import get_session_data, get_prompt_conversation_cache, set_prompt_conversation_cache, set_query_cache, get_query_cache, set_graph_cache, get_graph_cache, set_idata_cache, get_idata_cache, remove_limit_fetch
from helper_methods import check_substring_single_space, format_column_name,getmd5hash, normalize_spaces, delcache, delsessioncache, clean_ending, init_cap, starts_with_delete_or_truncate, is_valid_email, check_graphing_request_by_string
...
def query(prompt: str, domain: str, usergroup: str, userid: str, session_id:str, followup_flg:str, previous_prompt: str, graphFlag:bool, conversation, reworded: str, user_key, model_id, convo_seq_no):
    ...
    if check_substring_single_space(query_raw, "Please clarify."):
        ...
        executed_sql = ""
        generated_sql = ""
        dummyid = persist_log_data("L", llm_id, user_id, trust_id, trust_score, user_prompt, convo_prompt, convo_id, convo_seq_num,
                                   generated_sql, is_trusted, is_prompt_equiv, is_template_equiv, executed_sql, db_error_code, db_error_txt,
                                   is_authorized, is_clarify, is_action, action_type, parentid, user_feedback_code, user_feedback_txt)

        set_chat_cache(session_id + "lastid",str(dummyid))
        ### conversation ####
    ...
    try:
        with get_connection(rbac_user_id,  "" ) as connection:
            ...
            try:
                ...
                actiondid = persist_log_data("L", llm_id, user_id, trust_id, trust_score, user_prompt, convo_prompt, convo_id, convo_seq_num,
                                                   generated_sql, is_trusted, is_prompt_equiv, is_template_equiv, executed_sql, db_error_code, db_error_txt,
                                                   is_authorized, is_clarify, is_action, action_type, parentid,user_feedback_code, user_feedback_txt)
                try:
                    if db_debug == "Y":
                        persist_app_debug(actiondid,debug_txt)
                ...
                set_chat_cache(session_id + "lastid",str(actiondid))
                ...
                actiondid = persist_log_data("R", llm_id, user_id, trust_id, trust_score, user_prompt, convo_prompt, convo_id, convo_seq_num,
                                                   generated_sql, is_trusted, is_prompt_equiv, is_template_equiv, executed_sql, db_error_code, db_error_txt,
                                                   is_authorized, is_clarify, is_action, action_type, actiondid,user_feedback_code, user_feedback_txt)
        ...
        if fetchstmt:
            ...
            iquery = query_clean
        set_idata_cache(idataid, iquery)
    ...
    if graphFlag:
        ...
        record_count_html = ""
        if len(df) > 0:
            record_count_html = f"""<br/>Displaying top {len(df)} record(s) <a href="{idata_app_url}?idataId={idataid}">View Full Result Set</a>"""
        else:
