...
resource "oci_core_instance" "vm" {
  ...
  compartment_id      = var.compartment_id
  display_name        = "nle-test-vm"
  shape               = "VM.Standard.E4.Flex"

  shape_config {
    ocpus         = 2         # Customize as needed
    memory_in_gbs = 32        # Customize as needed
  }
   create_vnic_details {
    subnet_id        = var.subnet_id
    display_name     = "primary-vnic"
    assign_public_ip = true
    hostname_label   = "nle-testvm"
  }
  ...
  metadata = {
    ...
    user_data = base64encode(templatefile("${path.module}/nl_engine_cloud_init.yml.tpl", {
