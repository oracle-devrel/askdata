...
@router.get("/admin_directories")
async def admin_directories():
    """
    Returns information including the real directory location
    """
    ...
    real_dir_pathlib = Path.cwd().resolve()
    
    return {
        "current_directory": current_dir,
        "real_directory": real_dir,
        "real_directory_pathlib": str(real_dir_pathlib),
...
@router.post("/update_certify_queue_null_duplicate_flags")
async def update_certify_queue_null_duplicate_flags(mode):
    logger.info(f"Update certify queue null duplicate flags {mode} entry")
    ret= ctrl.update_certify_queue_null_duplicate_flags(mode=mode)
    logger.info(f"Update certify queue null duplicate flags {mode} exit")
    return ret

@router.post("/update_certify_state_null_prompt_vects")
async def update_certify_state_null_prompt_vects(mode):
    logger.info(f"Update certify state null prompt vects {mode} entry")
    ret= ctrl.update_certify_state_null_prompt_vects(mode=mode)
    logger.info(f"Update certify state null prompt vects {mode} exit")
    return ret
