...
class fine_tune_controller:
    ...
    def configure_finetune(self,request:dict):
        ...
        request["unit_count"]=request.get("unit_count",confb.dconfig["oci"]["finetune"]["unit_count"])
        request["unit_shape"]=request.get("unit_shape",confb.dconfig["oci"]["finetune"]["unit_shape"])
        df = confb.dconfig["app"]["datetime_format_nospace"]
        request["workflow_key"] = f"{request['workflow_key']}_{now.strftime(df)}"
    ...
    async def create_finetune_DAC(self,workflow_key: str):
        ...
        if "compartment_ocid" not in request:
            request["compartment_ocid"]=walkback_d(confb.dconfig["oci"]["os"]["finetune"],"compartment_ocid")
        ...
        if "dac_error_dtls" not in dac_answer:
            ...
            self.ftdb_dao.update(
                ...
                req ={
                    "dac_submit_time": dac.get("dac_submit_time",""),
                    "dac_created_time":dac.get("dac_created_time",""),
                    "dac_lifecycle_state": dac.get("dac_lifecycle_state",""),
                    "dac_cluster_id": dac.get("dac_cluster_id",""),
                    "dac_unit_count": dac.get("dac_unit_count",""),
                    "dac_unit_shape": dac.get("dac_unit_shape",""),
                    "dac_error_dtls": ""
                    })
        else:
            req = {
    ...
    async def run_finetune(self,workflow_key: str):
        ...
        Step 1.4
        """
        db_response :dict = self.ftdb_dao.get_by_key(workflow_key=workflow_key)
        ...
        fn_workflow = db_response
        logging.info(fn_workflow)

        #1. Preconditions check
        if len(fn_workflow.get("dac_cluster_id","")) == 0:
            raise HTTPException(
                status_code=409,
                detail={
                        "status": "error",
        ...
        if fn_workflow.get("dac_lifecycle_state","").lower() != 'active':
            ls=fn_workflow.get("dac_lifecycle_state","").lower()
            raise HTTPException(
                status_code=409,
                detail={
                        "status": "error",
                        "message": f"The {workflow_key} DAC is not in a proper state [{ls}] for the requested operation."
                        }
        ...
        req={
             "compartment_ocid": walkback_d(confb.dconfig["oci"]["finetune"],"compartment_ocid"),
             "ft_base_model_ocid": confb.dconfig["oci"]["finetune"]["base_model_id"],
             "dataset_type": confb.dconfig["oci"]["finetune"]["dataset_type"],
             "namespace":walkback_d(confb.dconfig["oci"]["finetune"],"namespace"),
             "bucket": walkback_d(confb.dconfig["oci"]["os"]["finetune"],"bucket"),
             "object_name": fn_workflow.get("training_data_path",""),
             "cluster_ocid": fn_workflow.get("dac_cluster_id",""),
    ...
    def deploy_finetune_model(self, workflow_key:str):
        ...
        old_model : dict  = self.model_dao.select_purpose(model_purpose="GEN-PURPOSE-LLM")
        old_model["usage_stop"]=datetime.datetime.now()
        
        req = {}
        req["compartment_ocid"]=confb.dconfig["oci"]["compartment_ocid"]
...
class llm_metrics_controller:
    ...
    def accuracy_by_week(self, start_date=None, end_date=None, domain=None):
        logger.info("Service_Helper::Accuracy by week")
        return metric.accuracy_by_week(start_date=start_date, end_date=end_date,domain=domain)
